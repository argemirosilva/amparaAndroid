...
class AudioTriggerSingleton {
  ...
  private processAudioFrame(samples: Float32Array, sampleRate: number) {
    ...
    this.frameBuffer.push({ ...frameMetrics, timestamp: now });
    ...
    if (this.frameBuffer.isFull && now - this.lastAggregationTime >= 500) {
      ...
      const f0 = pitchResult.f0;
      const f0Median = getF0Median2s();
      const gender = classifyGender(f0Median, pitchResult.confidence, speechDensity > 0.3, this.config);
      ...
      this.metrics = {
        ...
        recordingOn: this.triggerState === 'RECORDING',
        recordingDuration: 0,
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (!this.adaptiveNoiseFloor) {
        ...
        const learningRate = this.config.noiseFloorLearningRate ?? 0.1;
  ...
  setCalibrationStatus(calibrated: boolean) {
    ...
    this.isCalibrated = calibrated;
    // If we receive calibration, it means we ARE capturing
    this.isCapturing = true;
    this.notifyStateChange();
  }
  ...
  setIsCapturing(capturing: boolean) {
    console.log('[AudioTriggerSingleton] Setting capture status:', capturing);
  ...
  setNativeMetrics(nativeMetrics: any) {
    // Sync calibration status if provided by native
    if (nativeMetrics.isCalibrated !== undefined) {
      if (this.isCalibrated !== nativeMetrics.isCalibrated) {
        console.log('[AudioTriggerSingleton] 🔄 Syncing calibration from native:', nativeMetrics.isCalibrated);
        this.setCalibrationStatus(nativeMetrics.isCalibrated);
      }
    }
    ...
    this.metrics = {
      ...
      recordingOn: this.getIsRecording(),
      recordingDuration: 0,
      state: this.triggerState,
      isNoisy: false,
    };
